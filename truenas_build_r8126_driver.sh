#!/bin/bash
# Script to compile r8126 driver for TrueNAS SCALE using Docker
#
# Usage:
#   1. Copy this script to your TrueNAS host.
#   2. Run: chmod +x truenas_build_r8126_driver.sh && sudo ./truenas_build_r8126_driver.sh
#
# Prerequisites:
...
#      cp /tmp/r8126_build/rtl8126/r8126.ko /mnt/YOUR_POOL/scripts/
#   2. Go to System Settings -> Advanced -> Init/Shutdown Scripts.
#   3. Add a 'Post Init' script with command: insmod /mnt/YOUR_POOL/scripts/r8126.ko
#
...
#   - I have tested on my TrueNAS SCALE 25.10.1 box with a Realtek RTL8126 NIC where it works as intended.

set -euo pipefail

BUILD_DIR="${BUILD_DIR:-/tmp/r8126_build}"
IMAGE_NAME="${IMAGE_NAME:-r8126-builder}"
REPO_URL="${REPO_URL:-https://github.com/openwrt/rtl8126.git}"
...
prepare_build_env() {
    ...
    if [ ! -d "$SRC_DIR_NAME" ]; then
        log "Cloning r8126 driver source..."
...
install_and_verify() {
    local module_path="$BUILD_DIR/$SRC_DIR_NAME/r8126.ko"

    if [ ! -f "$module_path" ]; then
        die "Compilation failed. r8126.ko was not generated."
    fi
    ...
    if [ -d /sys/module/r8126 ] || lsmod | grep -qE '^r8126[[:space:]]'; then
        log "Module r8126 is already loaded. Attempting to unload..."
        rmmod r8126 || die $'Could not unload r8126. Is the interface in use?\nTry bringing the interface down before running this script.'
    fi

    log "Loading new module..."
    local insmod_err=""
    if ! insmod_err=$(insmod "$module_path" 2>&1); then
        if [ -d /sys/module/r8126 ] && printf '%s' "$insmod_err" | grep -qi "file exists"; then
    ...
    log "Recent kernel messages containing 'r8126':"
    dmesg | tail -n 200 | grep -i "r8126" || true
    ...
    if [ -d /sys/class/net ]; then
        ...
        for iface in /sys/class/net/*; do
            ...
            if [ -e "$iface/device/driver/module" ]; then
                ...
                if [ "$module_name" = "r8126" ]; then
                    ifaces_using_driver+=("$iface_name")
                fi
            fi
        done
        if [ "${#ifaces_using_driver[@]}" -gt 0 ]; then
            log "Interfaces using r8126: ${ifaces_using_driver[*]}"
        else
            log "No interfaces currently report driver module r8126 via sysfs."
        fi
    fi
